...
class UserRepository(private val database: AppDatabase) {
    ...
    fun fetchUsers(): Flow<PagingData<User>> {
        return Pager(
            config = PagingConfig(ITEMS_PER_PAGE, prefetchDistance = 1, enablePlaceholders = false),
            remoteMediator = UserRemoteMediator(this),
            pagingSourceFactory = { database.userDao.getUserForPagingSource() as PagingSource<Int, User> }
        ).flow
    }
}